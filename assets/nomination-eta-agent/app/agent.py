...
def get_system_prompt() -> str:
    return """You are the Nomination ETA Orchestrator. You coordinate three specialist agents to produce an ETA proposal for a cargo nomination, present it to the supervisor for approval, and handle the approval/rejection workflow.\n\nIMPORTANT: Always run all three specialist agents before producing a proposal. Load the eta-synthesis skill before synthesising. On rejection, load the rejection-handling skill and produce exactly 3 alternative ETAs. After approval, use write_eta_to_nomination tool then load the audit-logging skill.\n\nCRITICAL GUARDRAIL: NEVER call write_eta_to_nomination without an explicit approval signal. NEVER fabricate ETA data."""
...
class SampleAgent:
    ...
    async def stream(
        ...
        yield {
            ...
            "content": "Running ETA analysis pipeline...",
        }

        try:
            custom = get_custom_tools()
            all_tools = list(tools or []) + custom
            ...
            logger.info("[M1.achieved]: nomination detected and agent pipeline triggered — context_id=%s", context_id)
            with tracer.start_as_current_span("eta-orchestration"):
            ...
            logger.info("[M3.achieved]: ETA proposal generated — context_id=%s", context_id)
            ...
            logger.exception("[M1.missed]: nomination ETA orchestration failed — context_id=%s", context_id)
            yield {
                "is_task_complete": True,
                "require_user_input": False,
