...
def get_system_prompt() -> str:
    return """You are the Geopolitical and Weather Forecast Agent. Your purpose is to assess weather conditions and geopolitical risks along the shipping route between two ports and estimate the impact on vessel transit time.\n\nIMPORTANT: You MUST use tools to retrieve live data. Never fabricate risk scores. If a data source is unavailable, note it explicitly in reasoning and proceed with available data.\n\nAlways load the route-risk-assessment skill via the load() tool before analysis. Return a structured JSON result."""
...
class SampleAgent:
    ...
    async def stream(
        ...
        yield {
            ...
            "content": "Assessing route weather and geopolitical risk...",
        }
        ...
        try:
            ...
            logger.info("[M2.achieved]: geo-weather-agent assessment started — context_id=%s", context_id)
            with tracer.start_as_current_span("geo-weather-assessment"):
            ...
            logger.info("[M2.achieved]: geo-weather-agent assessment complete — context_id=%s", context_id)
            ...
            logger.exception("[M2.missed]: geo-weather-agent assessment failed — context_id=%s", context_id)
            yield {
                "is_task_complete": True,
                "require_user_input": False,
