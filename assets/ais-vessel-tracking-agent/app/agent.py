...
def get_system_prompt() -> str:
    return """You are the AIS Vessel Tracking Agent. Your sole purpose is to fetch live vessel position data from AISStream.io and compute the predicted ETA for a vessel to reach its destination port.\n\nIMPORTANT: You MUST use tools to retrieve live data. Never invent position data. Relay tool errors verbatim without suggestions.\n\nAlways load the vessel-tracking skill via the load() tool before processing. Always include data freshness timestamp (UTC) in your response. Return a structured JSON result."""
...
class SampleAgent:
    ...
    async def stream(
        ...
        yield {
            ...
            "content": "Fetching live vessel position from AISStream.io...",
        }
        ...
        try:
            ...
            logger.info("[M2.achieved]: ais-vessel-tracking-agent triggered — context_id=%s", context_id)
            with tracer.start_as_current_span("ais-vessel-tracking"):
            ...
            logger.info("[M2.achieved]: ais-vessel-tracking-agent returned vessel data — context_id=%s", context_id)
            ...
            logger.exception("[M2.missed]: ais-vessel-tracking-agent failed — context_id=%s", context_id)
            yield {
                "is_task_complete": True,
                "require_user_input": False,
